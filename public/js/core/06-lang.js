...
const I18N_RULES = [
  ...
  [/^Belum ada kebiasaan di (.+)$/i, 'No habits in $1'],
  [/^Tidak ada hasil untuk (.+)$/i, 'No results for $1'],
  [/^Belum ada catatan untuk (.+)$/i, 'No notes for $1'],
  ...
  [/^Target "(.+)" dibuat\.$/i, 'Target "$1" created.'],
  [/^Kategori “(.+)” dibuat\.$/i, 'Category “$1” created.'],
  [/^(\d+) dokumen diimpor\.$/i, '$1 documents imported.'],
  [/^Rutinitas "(.+)" selesai hari ini\.$/i, 'Routine "$1" done for today.'],
  [/^Penarikan melebihi saldo (.+)\.$/i, 'Withdrawal exceeds the $1 balance.'],
  [/^Reset semua centang untuk (.+)\? Nama kebiasaan tetap disimpan\.$/i, 'Reset all ticks for $1? The habit name is kept.'],
  [/^Berkas terlalu besar \((.+)\)\. Maks 2 MB\.$/i, 'File too large ($1). Max 2 MB.'],
  [/^Hapus "(.+)" dari (.+)\?$/i, 'Delete "$1" from $2?'],
  [/^Hapus budget "(.+)" \((.+)\)\?$/i, 'Delete budget "$1" ($2)?'],
  [/^Hapus dokumen “(.+)”\?$/i, 'Delete document “$1”?'],
  [/^Hapus project "(.+)"\? Task yang terhubung tidak ikut terhapus\.$/i, 'Delete project "$1"? Linked tasks are not deleted.'],
  [/^Hapus tabungan "(.+)" beserta riwayat setoran \((.+) catatan\)\?$/i, 'Delete savings "$1" with its deposit history ($2 records)?'],
  [/^Hapus transaksi "(.+)" \((.+)\)\?$/i, 'Delete transaction "$1" ($2)?'],
  [/^Lewat (\d+) hari$/i, '$1 days overdue'],
  [/^Skor gabungan (.+)$/i, 'Combined score $1'],
  [/^nilai (.+) dari (.+) poin$/i, 'value $1 of $2 points'],
  [/^tugas \((\d+)\)$/i, 'tasks ($1)'],
  [/^(.+)% dari$/i, '$1% of'],
  [/^Setoran ke (.+)$/i, 'Deposit to $1'],
...
const I18N_WORDS = {
  ...
  'tanggal': 'date', 'fokus': 'focus', 'ini': 'this', 'tinggi': 'high', 'sedang': 'medium', 'rendah': 'low',
