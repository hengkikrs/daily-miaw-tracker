...
async function adminFetch(ctx, path, init = {}) {
  ...
  let data = null;
  try { data = text.trim() ? JSON.parse(text) : null; } catch { /* non-json */ }
...
// Delete any account whose 24h deletion window has elapsed. Returns count purged.
...
export default async function handler(req, res) {
  ...
  try { await purgeExpired(ctx); } catch { /* best-effort sweep */ }
  ...
  let userId = '';
  try { userId = String(JSON.parse(body || '{}').userId || ''); } catch { /* ignore */ }
  ...
  if (!callerToken) return json(res, 401, { error: 'unauthorized' });
  const who = await adminFetch({ base: ctx.base, headers: { apikey: ctx.head...ey, Authorization: 'Bearer ' + callerToken } }, '/auth/v1/user');
  if (!who.ok || !who.data || String(who.data.id || who.data.aud || '').toLowerCase() !== userId.toLowerCase()) {
  ...
  return json(res, 200, { ok: true, deletion_requested_at: stamp, delete_after: new Date(Date.parse(stamp) + DAY_MS).toISOString() });
}
